...
// value for the weapons accuracy, more this value is less accuracy weapons have
...
const AT4 = { name:"AT4", id:302, fireRate:3, recoil:3, bulletSpeed:6, hasExplosiveBullets:true, bulletsExplosionRadius:4, bulletsArray:[], accuracy:3.5, zoomLevel:60, sound:"AT4_and_M72LAW_and_Panzerfaust3Shoot.ogg", texture:"cauldron", ammo:1, smoke:4, recipe:[
	"   ",
	"iri",
	"   "] };
...
const BARRETT_EXPLOSIVE = { name:"Barrett Explosive", id:304, fireRate:12, recoil:22, bulletSpeed:9, hasExplosiveBullets:true, bulletsExplosionRadius:3, bulletsArray:[], accuracy:3, zoomLevel:60, sound:"BarrettShoot.ogg", texture:"comparator", ammo:10, smoke:2, recipe:[
	"   ",
	"iri",
	"   "] };
...
function shootArrow(gun)
{
	...
	var accuracyValueX = ((Math.random() * randomness) - (randomness / 2)) * gun.accuracy * gunShootDir.z * gunShootDir.z;
	var accuracyValueY = ((Math.random() * randomness) - (randomness / 2)) * gun.accuracy;
	var accuracyValueZ = ((Math.random() * randomness) - (randomness / 2)) * gun.accuracy * gunShootDir.x * gunShootDir.x;
	var arrow = Level.spawnMob(getPlayerX() + (gunShootDir.x * 2), getPlayerY() + (gunShootDir.y * 2.5), getPlayerZ() + (gunShootDir.z * 2), 80);
	setVelX(arrow, gunShootDir.x * ((1 + accuracyValueX) * gun.bulletSpeed));
	setVelY(arrow, gunShootDir.y * ((1 + accuracyValueY) * gun.bulletSpeed));
	setVelZ(arrow, gunShootDir.z * ((1 + accuracyValueZ) * gun.bulletSpeed));
	...
	currentActivity.runOnUiThread(new java.lang.Runnable(
	{
		run: function()
		{
			try
			{
				...
				var settingsImageSquareLengthScaled = Math.floor(displayHeight / 10);
				...
				var imageShotHeightScaled = Math.floor(displayHeight / 15);
				...
				firePngScaled = new android.graphics.Bitmap.createBitmap(firePngDecoded, 0, 0, imageShotWidth, imageShotHeight, matrix2, true);
